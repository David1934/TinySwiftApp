...
#if (ADS6401_MODULE_SPOT == SWIFT_MODULE_TYPE)
    vcsel_opn7020_settings = (struct setting_rvd *) mapped_script_vcsel_opn7020_settings;
    memset(mapped_script_vcsel_opn7020_settings, 0, sizeof(struct setting_rvd)*MAX_REG_SETTING_COUNT);
#else
    vcsel_PhotonIC5015_settings = (struct setting_r16vd *) mapped_script_vcsel_PhotonIC5015_settings;
    memset(mapped_script_vcsel_PhotonIC5015_settings, 0, sizeof(struct setting_r16vd)*MAX_REG_SETTING_COUNT);
#endif
...
#if (ADS6401_MODULE_SPOT == SWIFT_MODULE_TYPE)
            else if (VCSEL_OPN7020_I2C_ADDR_IN_SCRIPT == pstrItems[i].i2c_addr)
            {
...
#else
            else if (MCUCTRL_I2C_ADDR_4_FLOOD_IN_SCRIPT == pstrItems[i].i2c_addr)
            {
...
#if (ADS6401_MODULE_SPOT == SWIFT_MODULE_TYPE)
                else if (VCSEL_OPN7020_I2C_ADDR_IN_SCRIPT == pstrItems[i - 1].i2c_addr)
                {
...
#else
                else if (MCUCTRL_I2C_ADDR_4_FLOOD_IN_SCRIPT == pstrItems[i - 1].i2c_addr)
                {
                    if (vcsel_reg_setting_cnt > 1)
                    {
...
#if (ADS6401_MODULE_SPOT == SWIFT_MODULE_TYPE)
bool Misc_Device::check_crc_4_eeprom_item(uint8_t *pEEPROMData, uint32_t offset, uint32_t length, uint8_t savedCRC, const char *tag)
{
    bool ret = true;
...
int Misc_Device::check_crc32_4_dtof_calib_eeprom_param(void)
{
    int ret = 0;
    uint32_t offset;
    uint32_t length;
    ...
    if (!check_crc_4_eeprom_item(pEEPROMData, 0, eeprom_data_size - length, checkSum[CHECKSUM_ALL], "14.CHECKSUM_ALL")) {
        DBG_ERROR("14.Checksum checksum_all validation fail, sizeof(swift_eeprom_data_t)=%ld.\n", sizeof(swift_eeprom_data_t));
        ret = -EIO;
    }
